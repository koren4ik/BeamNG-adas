...
from visualizer import Visualizer            # Визуализация (опционально)
...
class ADASController:
    def __init__(self, vehicle, sensors_bundle):
        ...
        # Последний кадр и результат детекции — для визуализатора
    ...
    def measure(self, t_now: float, leader_stopped: bool) -> Measurements:
        ...
        if radar_data is not None and radar_data.size > 0:
            ...
            # Цели нет — фильтр НЕ подкармливаем мусором. Просто отдаём большое расстояние и нулевой доплер, has_target=False.
        ...
        # Сохраняем frame_rgb и lane_result в self чтобы визуализатор (если
        # включён) мог их использовать. Это альтернатива расширению Measurements.
        self.last_frame_rgb = None
        self.last_lane_result = None
        try:
    ...
    def compute_steering(self, m: Measurements) -> float:
        ...
        # — ВЫБОР SETPOINT —
        # Когда lane_valid=True — рулим по смещению из центра полосы.
        # Когда lane_valid=False — error=0, PID плавно ослабляет руль.
        # ЗНАК: при lane_offset_m > 0 (машина вправо от центра) нужен steering < 0 для возврата --- error = -lane_offset_m.
        if m.lane_valid:
            error = -m.lane_offset_m
        else:
            error = 0.0
...
def main():
    ...
    try:
        while True:
            ...
            # Визуализация (если включена). update() сама throttled до 20 Гц,
            # не каждый тик рисует. Берёт frame и lane_result из adas.
            viz.update(adas.last_frame_rgb, adas.last_lane_result,

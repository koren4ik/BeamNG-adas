...
#  КОНСТАНТЫ
# ─────────────────────────────────────────────
LEADER_BASE_SPEED  = 110 / 3.6
LEADER_AMPLITUDE   = 20 / 3.6
LEADER_PERIOD      = 10.0
LEADER_STOP_AT     = 30.0          # секунд до остановки лидера

TARGET_SPEED       = 130 / 3.6    # крейсерская скорость
TIME_HEADWAY       = 2.0           # секунды — дистанция = speed * headway
MIN_SAFE_DIST      = 10.0          # минимальная дистанция даже на малой скорости
...
def next_state(current, distance, doppler, ttc, us_min, speed, aeb_ticks, leader_stopped):
    ...
    Возвращает (новое_состояние, новый_счётчик_aeb).
    """
...
while True:
    ...
    # ── FOLLOW ──────────────────────────────
        ...
        if not leader_stopped:
            ...
            if dist_error > 0:
                correction = min(dist_error*0.5 , 30.0)  # к скорости лидера
    ...
    # ── CREEP ───────────────────────────────
        ...
        # Санити-чек радара внутри CREEP:
        # Если радар показывает близко, но US ещё молчит (>4м) — радар врёт.
        # В этом случае используем US как единственный источник истины:
        # едем на 5 км/ч пока US не активируется.
        radar_lying = (distance < 4.0 and front_dist > 4.0)

        print(_fmt_log("CREEP", [
            ("speed",    f"{speed*3.6:.1f} км/ч"),
            ("radar_s",  f"{distance:.1f} м{'  [BAD]' if radar_lying else ''}"),
        ...
        # Фаза 1 — гасим скорость до CREEP_BRAKE_SPEED (5.5 км/ч)
        ...
        # Фаза 2 — ползём, стоп только по US
        else:
            us_stop = front_dist < US_STOP_DIST or rear_dist < US_STOP_DIST

            # radar_stop срабатывает ТОЛЬКО если радар достоверен (US подтверждает)
            radar_stop = (not radar_lying) and leader_stopped and distance < CREEP_RADAR_STOP
            ...
            if us_stop or radar_stop:
                ...
                # Если радар врёт — ползём на фиксированных 5 км/ч до активации US.
                # Если радар честный — используем его для пропорционального замедления.
                if radar_lying:
                    target_creep = US_CREEP_SPEED
                else:

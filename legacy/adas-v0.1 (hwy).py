...
while True:
    ...
    if elapsed > 25:
        ...
        prev_leader_x = leader_x  # Обновляем для следующего кадра

            # Уменьшенные коэффициенты и минус перед формулой (если ось X вправо, а руль влево - это минус)
    ...
    ego_x = pos_sensor.data.get('pos', (0, 0, 0))[0]
    dx = ego_x - prev_ego_x  # скорость отклонения
    prev_ego_x = ego_x  # Обновляем для следующего кадра

    # Смягченные коэффициенты (P=0.05, D=0.5). Обрати внимание на минус!
    ...
    if use_ultrasonic:
        ...
        if front_dist < US_STOP_DIST or rear_dist < US_STOP_DIST:
            ...
            US_CREEP_i_error += creep_error * 0.05  # Интегрируем (0.1 - это шаг времени)

            # Ограничиваем накопленную ошибку (Anti-windup), чтобы машина не "взлетала"
            US_CREEP_i_error = max(-1.0, min(1.0, US_CREEP_i_error))

            # Новая формула управления
            # Kp = 0.15 (чуть агрессивнее на скорость), Ki = 0.05 (плавное дотягивание)
